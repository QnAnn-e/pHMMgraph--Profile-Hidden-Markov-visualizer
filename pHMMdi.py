...
parser.add_argument('-f', '--format', action='store', help='specify a format for the diagram output')
...
# if not os.path.isdir(user_specified_path):
#     print('Path does not exist')
#     sys.exit()
# if not os.path.isdir(user_specified_datatype):
#     print('A datatype is required')
#     sys.exit()
# if not os.path.isdir(user_specified_query):
#     print('A query sequence is required')
#     sys.exit()
...
class Algorithms:
    ...
    def viterbi(self, query, bi_o):
        ...
        aMjIj = self.transition[next_state][2]
        aDj_1Dj = self.transition[state][3]
        aDj_1Mj = self.transition[state][4]
        aDjIj = self.transition[next_state][5]
        aIjIj = self.transition[next_state][6]
        ...
        for st in range(0, (len(self.transition) - 1)):
            ...
            log_values = []
            state += 1
            if next_state < len(self.transition) - 1:
                next_state += 1
            aMj_1Mj = self.transition[state][0]
            aMj_1Dj = self.transition[state][1]
            aMjIj = self.transition[next_state][2]
...
class Model:
    ...
    def viterbi_vizualizer(self, pad):
        ...
        next = 1
        x = 2
        for state in range(0, (len(pad2) - 2)):
            t_size = 'x-small'
            if len(match) > 12:
                t_size = 'xx-small'
            rectangle = plt.Rectangle((x - 1, 3), width=2, height=1, fc='None', ec="black", linewidth=1.5)
        ...
        for state in range(0, (len(pad2)-2)):
            ...
            if state == (len(pad2)-1):
                ...
                if state == (len(pad2) - 3):
                    ...
                    axes.plot([x-1], [7.3], marker=r'$\circlearrowright$', ms=size, color=colour_ii)
            if state == 0 or state == (len(pad2)-2):
                pass
            else:
                if state == (len(pad2) - 3):
