...
parser = argparse.ArgumentParser(prog='pHMMgraph', usage='%(prog)s [options] path',
                                 description='pHMMgraph is a profile Hidden Markov Modeller that provides the user with '
                                             ...
                                             'and Viterbi algorithms as well the transition and emissions tables. ',
...
class ReadAlignment:
    ...
    def read_file(self, filepath):
        ...
        if (counter == amount_of_seq):
            print("Thanks, all the sequences are the same length.")
            print("Step one is complete :) You're doing great!")
...
# So this creates an array containing the current states of the alignment
    def current_states(self, match, df_table):
        ...
        i = 0
        j = 0
        if df_table.columns[0] == match[0]:
            for nucl in df_table[0]:
                array_row.append("M")
        else:  # if first column is not an insert
            for nucl in df_table[0]:
                array_row.append("I")
    ...
    def transition_probs(self, new_array):
        ...
        for s in range(len(new_array)-1):  # loop through all the alignments one by one
            ...
            if 'I' in previous:  # meaning there was an insertion state and a temp list was created
                current = previous
                if 'I' in next:  # if it goes to another insertion
                    for a in current:
            ...
            im = 0
            id = 0
        # print("first transition table: ", total_iterations)
        ...
        for state in concatenated:  # loop through all states
            ...
            for transition in state:  # loop through every state in the transition maw mm until id
                ...
                if index1 <= 2:  # for states mm, md, mi
                    ...
                    if index2 == 0:
                        temp_transition_array.append(0)
                        index1 += 1
                    else:
        ...
        for emission in query:
            ...
            if sample_type == 'N':
                ...
                if emission_o == 'A':
                    ...
                    bi_o.append(self.emissions[4][s])
                elif emission_o == 'F':
                    bi_o.append(self.emissions[5][s])
                elif emission_o == 'G':
                    bi_o.append(self.emissions[6][s])
                elif emission_o == 'H':
                    bi_o.append(self.emissions[7][s])
                elif emission_o == 'I':
                    bi_o.append(self.emissions[8][s])
                elif emission_o == 'K':
                    bi_o.append(self.emissions[9][s])
                elif emission_o == 'L':
                    bi_o.append(self.emissions[10][s])
                elif emission_o == 'M':
                    bi_o.append(self.emissions[11][s])
                elif emission_o == 'N':
                    bi_o.append(self.emissions[12][s])
                elif emission_o == 'P':
                    bi_o.append(self.emissions[13][s])
                elif emission_o == 'Q':
                    bi_o.append(self.emissions[14][s])
                elif emission_o == 'R':
                    bi_o.append(self.emissions[15][s])
                elif emission_o == 'S':
                    bi_o.append(self.emissions[16][s])
                elif emission_o == 'T':
                    bi_o.append(self.emissions[17][s])
                elif emission_o == 'V':
                    bi_o.append(self.emissions[18][s])
                elif emission_o == 'W':
                    bi_o.append(self.emissions[19][s])
                elif emission_o == 'Y':
                    bi_o.append(self.emissions[20][s])
                elif emission_o == 'Z':
                    bi_o.append(self.emissions[21][s])
                else:  # emission_o == '-'
...
class Model:
    ...
    def transition_visualizer(self, transition, pad):
        colour = "black"
        ...
        if pad[0] == 'match':
            width_mm = transition[0][0] *3
        if pad[0] == 'insert':
            width_im = transition[0][7] *3
    ...
    def viterbi_vizualizer(self, pad):
        ...
        next = 1
        x = 2
        for state in range(0, (len(pad) - 1)):
            t_size = 'x-small'
            if len(match) > 12:
                t_size = 'xx-small'
            rectangle = plt.Rectangle((x - 1, 3), width=2, height=1, fc='None', ec="black", linewidth=1.5)
...
if __name__=='__main__':
    ...
    # this is what happens when I merge stuff
    ...
    model = Model()
    model.transition_visualizer(transition_array, best_path_list)
